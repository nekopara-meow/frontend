<template>
  <div class="file-pre-container">
    ...
    <div class="file-info bluelight">
      <div style="font-size: 15px">{{ fileName }}</div>
      <div>{{ author }}</div>
      <div>{{ modDate }}</div>
    </div>
    <ul
    ...
    <el-dialog v-model="dialogFormVisible" :title="createFileTitle">
      <el-form :model="fileInitial">
        ...
        <el-form-item
          label="选择文件模板"
          v-if="this.file_type === 1"
          label-width="140px"
        >
          <el-select
            v-model="doc_model_id"
            class="m-1"
            placeholder="不选择模板"
            size="large"
          >
...
export default {
  ...
  methods: {
    ...
    openFile() {
      ...
      if (this.fileInitial.name === null) return;
      switch (this.file_type) {
        case 0:
          console.log({
            username: this.username,
